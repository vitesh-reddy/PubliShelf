...
const mockBooks = [
  {
    ...
    author: "Vitesh Reddy",
    description:
      "A chilling thriller that delves into the dark corners of the mind. Follow the protagonist as they navigate a world filled with secrets, betrayals, and a shadowy past that refuses to stay buried. Each chapter pulls you deeper into a web of suspense, leaving you guessing until the very end.",
    genre: "Thriller",
    price: 1090,
    quantity: 11,
    image:
  ...
  },
  {
    ...
    quantity: 15,
    image:
      "https://images.unsplash.com/photo-1514894780887-121968d00567?w=800&q=80",
    rating: 4.8,
  },
  {
...
function insertMockData() {
  mockBooks.forEach((book) => {
    db.get(
      "SELECT * FROM books WHERE bookTitle = ? AND author = ?",
      ...
      (err, row) => {
        if (err) {
          ...
          db.run(
            `INSERT INTO books (bookTitle, author, description, genre, price, quantity, image, rating)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [
              book.bookTitle,
              book.author,
